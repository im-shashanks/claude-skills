...
- Place persisted tests alongside existing tests with agent-specific suffixes to prevent collisions:
  - Input/boundary probes: `_adversarial_input` suffix (e.g., `test_auth_adversarial_input.py`)
  - Fault/resilience probes: `_adversarial_fault` suffix (e.g., `test_auth_adversarial_fault.py`)
