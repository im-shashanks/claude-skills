...
The quality loop is Shaktra's reusable retry pattern for quality gates. When a reviewer finds issues, the creator agent fixes them and the reviewer re-checks. After 3 failed attempts, the loop escalates to the user. This pattern is used at every TDD gate and during TPM design review.
...
- On **BLOCKED**, the creator agent receives findings and applies fixes, then the reviewer runs again.
